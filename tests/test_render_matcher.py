...
# Paths use forward slashes so os.path splits them the same on Windows and
# POSIX (Windows os.path accepts '/' too), keeping these unit tests portable.
...
class TestNormalization(unittest.TestCase):
    ...
    def test_unicode_fold(self):
        self.assertEqual(rm.normalize_strict('Café'), rm.normalize_strict('café'))
...
class TestMatching(unittest.TestCase):
    def test_exact_name_same_folder(self):
        p = project('C:/FL/My Beat.flp')
        a = audio('C:/FL/My Beat.wav', duration=180)
    ...
    def test_loose_match_render_subfolder(self):
        p = project('C:/FL/My Beat.flp')
        a = audio('C:/FL/Rendered/My Beat master 140 Fmin.mp3', duration=182)
        score, _ = rm.score_pair(a, p)
        self.assertGreaterEqual(score, rm.MATCH_THRESHOLD)

    def test_stem_folder_named_after_project(self):
        p = project('C:/FL/My Beat.flp')
        a = audio('C:/FL/My Beat/Master.wav', duration=181)
        score, reason = rm.score_pair(a, p)
        self.assertGreaterEqual(score, rm.MATCH_THRESHOLD)
        self.assertIn('folder', reason)

    def test_input_sample_is_not_a_render(self):
        p = project('C:/FL/Kick.flp', samples=['C:/Samples/kick.wav'])
        a = audio('C:/Samples/kick.wav', duration=0.5, used_as_sample=True)
        score, _ = rm.score_pair(a, p)
    ...
    def test_reimported_render_still_matches_own_project(self):
        ...
        a_proj = project('C:/FL/kick.flp')
        a = audio('C:/FL/kick.wav', duration=190, used_as_sample=True)
        score, _ = rm.score_pair(a, a_proj)
        self.assertGreaterEqual(score, rm.MATCH_THRESHOLD)

    def test_generic_needs_proximity(self):
        p = project('C:/FL/Untitled.flp')
        near = audio('C:/FL/Untitled.wav', duration=180)
        far = audio('D:/Random/Untitled.wav', duration=180)
    ...
    def test_short_duration_penalized(self):
        p = project('C:/FL/My Beat.flp')
        a = audio('C:/FL/My Beat.wav', duration=0.5)  # a 0.5s "render" is doubtful
        long = audio('C:/FL/My Beat.wav', duration=180)
        self.assertLess(rm.score_pair(a, p)[0], rm.score_pair(long, p)[0])

    def test_prefix_does_not_overmatch_short_roots(self):
        p = project('C:/FL/beat.flp')  # generic + short
        a = audio('C:/FL/beatmania_full_mix.wav', duration=200)
        self.assertEqual(rm.score_pair(a, p)[0], 0)


class TestClassify(unittest.TestCase):
    def test_full_classify(self):
        projects = [project('C:/FL/My Beat.flp', samples=['C:/S/kick.wav']),
                    project('C:/FL/Untitled.flp')]
        audios = [
            audio('C:/FL/My Beat.wav', duration=180),
            audio('C:/FL/My Beat/Master.wav', duration=181),
            audio('C:/FL/Rendered/My Beat master.mp3', duration=182),
            audio('C:/S/kick.wav', duration=0.4, used_as_sample=True),
            audio('C:/Music/Exports/old track.wav', duration=200),
        ]
        matches, orphans = rm.classify(projects, audios)
        mybeat = matches['C:/FL/My Beat.flp']
    ...
    def test_long_sample_not_flooding_orphans(self):
        projects = [project('C:/FL/My Beat.flp')]
        audios = [audio('C:/Samples/ambient loop.wav', duration=90)]  # long, not render folder
        _, orphans = rm.classify(projects, audios)
        self.assertEqual(orphans, [])

    def test_include_long_surfaces_them(self):
        projects = [project('C:/FL/My Beat.flp')]
        audios = [audio('C:/Samples/ambient loop.wav', duration=90)]
    ...
    def test_accent_folded_match_via_classify(self):
        ...
        projects = [project('C:/FL/Cafe.flp')]
        audios = [audio('C:/FL/Café.wav', duration=180)]
        matches, _ = rm.classify(projects, audios)
        self.assertEqual(len(matches['C:/FL/Cafe.flp']), 1)

    def test_variant_dedup_count(self):
        recs = [audio('C:/FL/My Beat.wav', duration=180),
                audio('C:/FL/My Beat.mp3', duration=180),
                audio('C:/FL/My Beat 24bit.wav', duration=180)]
